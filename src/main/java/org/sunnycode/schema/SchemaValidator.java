...
import org.sunnycode.schema.types.TransformException;

/**
 * Validates schema instances for creation and upgrades. For validation of
 * instances against a schema, use TypeCompaction.
 */
public class SchemaValidator {
    private static final Pattern VALID_NAME_PATTERN = Pattern
            .compile("[a-zA-Z0-9_]+");

    public static void validate(Schema schemaDefinition)
    ...
    public static void validateAttributeUpgrade(Attribute oldAttr,
        ...
        case I8:
            assertAttributeTypeOneOf(name, newType, null, Type.I8, Type.I16,
                    Type.I32, Type.I64);
            break;
        case I16:
            assertAttributeTypeOneOf(name, newType, null, Type.I16, Type.I32,
                    Type.I64);
            break;
        case I32:
        ...
        case U8:
            assertAttributeTypeOneOf(name, newType, null, Type.U8, Type.U16,
                    Type.U32, Type.U64);
            break;
        case U16:
            assertAttributeTypeOneOf(name, newType, null, Type.U16, Type.U32,
                    Type.U64);
            break;
        case U32:
        ...
        case UTC_DATE_SECS:
            assertAttributeTypeOneOf(name, newType, null, Type.UTC_DATE_SECS,
                    Type.I64);
            break;

        case UTF8_SMALLSTRING:
            assertAttributeTypeOneOf(name, newType, null,
                    Type.UTF8_SMALLSTRING, Type.UTF8_TEXT);
            break;
    ...
    public static void assertEnumAttributeCompatible(Attribute oldAttr,
            Attribute newAttr) {
        if (newAttr == null) {
            throw new TransformException("enum attributes, such as '" + oldAttr
                    + "' may not be removed");
        }
        ...
        if (newValues == null || oldValues == null) {
            throw new TransformException("enum attribute '" + oldAttr.getName()
                    + "' has no values");
        }

        if (newValues.size() < oldValues.size()) {
            throw new TransformException("may not remove enum values from '"
        ...
        for (int i = 0; i < oldValues.size(); i++) {
            if (!oldValues.get(i).equals(newValues.get(i))) {
                throw new TransformException("illegal enum values update for '"
                        + oldAttr.getName() + "', " + newValues
                        + " updates in the front or middle of " + oldValues);
            }
        }
    }

    public static void assertAttributeTypeOneOf(String attrName, Type theType,
            Type... possibles) {
        ...
        throw new TransformException("attribute '" + attrName
