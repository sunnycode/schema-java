 ...
 */
package org.sunnycode.schema.compact;
...
import org.sunnycode.schema.Schema;
import org.sunnycode.schema.Transform;
import org.sunnycode.schema.compact.StructureTransform;
...
public class StructureTransformTest {
    ...
    Schema BOOLEAN_SCHEMA = (new Schema.Builder()).addAttribute("baz",
            Type.BOOLEAN, true).build();
    Schema DATE_SCHEMA = (new Schema.Builder()).addAttribute("quux",
            Type.UTC_DATE_SECS, true).build();
    ...
    public void testTransforms() {
        ...
        Map<String, Object> something = ImmutableMap.<String, Object> of("foo",
                "ONE", "bar", 1L, "baz", "true", "quux", now);
        Map<String, Object> somethingElse = ImmutableMap.<String, Object> of(
                "foo", "THREE", "bar", -1L, "baz", "false", "quux", then);
        ...
        Assert.assertEquals(bools.pack(something), ImmutableList.of(1L,
                ImmutableList.of("true"), ImmutableMap.<String, Object> of(
                        "foo", "ONE", "bar", 1L, "quux", now)));
        Assert.assertEquals(bools.pack(somethingElse), ImmutableList.of(1L,
                ImmutableList.of("false"), ImmutableMap.<String, Object> of(
                        "foo", "THREE", "bar", -1L, "quux", then)));
        ...
        Assert.assertEquals(dates.pack(something), ImmutableList.of(1L,
                ImmutableList.of(now), ImmutableMap.<String, Object> of("foo",
                        "ONE", "bar", 1L, "baz", "true")));
        Assert.assertEquals(dates.pack(somethingElse), ImmutableList.of(1L,
                ImmutableList.of(then), ImmutableMap.<String, Object> of("foo",
                        "THREE", "bar", -1L, "baz", "false")));
        ...
        Assert.assertEquals(enums.pack(something), ImmutableList.of(1L,
                ImmutableList.of("ONE"), ImmutableMap.<String, Object> of(
                        "bar", 1L, "baz", "true", "quux", now)));
        Assert.assertEquals(enums.pack(somethingElse), ImmutableList.of(1L,
                ImmutableList.of("THREE"), ImmutableMap.<String, Object> of(
                        "bar", -1L, "baz", "false", "quux", then)));
    }

    private static void assertRoundTrip(Transform transform, Object... objects) {
        for (Object object : objects) {
            Assert.assertEquals(transform.unpack(transform.pack(object)),
                    object);
        }
    }
}
